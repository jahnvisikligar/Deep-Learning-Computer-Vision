...
class Generator(nn.Module):
    ...
    def __init__(self, input_dims, output_dims):
        ...
        self.fc0 = nn.Sequential(nn.Linear(input_dims, 100))
...
def show_result(G_net, z_, num_epoch, show=False, save=False, path='result.png'):
    ...
    generated = G_net(z_).detach().cpu().numpy()
    ...
    for k in range(5 * 5):
        ...
        j = k % 5
        ax[i, j].cla()
        ax[i, j].imshow(np.reshape(generated[k], (28, 28)), cmap='gray')
    ...
    if num_epoch == 10 or num_epoch == 20 or num_epoch == 50 or num_epoch == 100:
        plt.savefig('result' + str(num_epoch) + '.png')
        plt.show()
